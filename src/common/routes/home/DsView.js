...
class DsView extends Component {
    constructor(props) {
        ...
        // RAG handlers
        this.handleAIIconClick = this.handleAIIconClick.bind(this);
        this.handleRAGConsent = this.handleRAGConsent.bind(this);
        this.handleCloseConsentModal = this.handleCloseConsentModal.bind(this);
    ...
    renderComplete () {
        ...
        // Render plotly graphs with async completion detection
        this.renderPlotlyInCells();
        // Wait for Plotly graphs to finish rendering before normalizing heights
        // Use setTimeout to ensure graphs have started rendering
        setTimeout(() => {
            this.waitForPlotlyRenderCompletion();
        }, 50);
    }
        ...
        plots.forEach((div) => {
            ...
            try {
                ...
                if (window.Plotly) {
                    ...
                    // Don't normalize heights here - wait for async render to complete
                } else {
    ...
    waitForPlotlyRenderCompletion() {
        const plots = document.querySelectorAll('.plotly-graph');
        if (plots.length === 0) {
            // No graphs, just normalize immediately
        ...
        // Create a Map to track which plots have rendered
        const plotsMap = new Map();
        plots.forEach(plot => plotsMap.set(plot, false));
        ...
        const observer = new MutationObserver((mutations) => {
            ...
            plots.forEach(plot => {
                // Check if plot has svg children (Plotly renders to SVG)
                const hasSvg = plot.querySelector('svg') !== null;
                const hasData = plot.querySelector('.scatterplot') !== null || 
                               plot.querySelector('.barlayer') !== null ||
                               plot.querySelector('text') !== null;
                
                if (hasSvg || hasData) {
                    plotsMap.set(plot, true);
                } else {
                    allRendered = false;
                }
            });

            // If all plots have rendered, normalize heights and stop observing
            if (allRendered && Array.from(plotsMap.values()).every(v => v === true)) {
                observer.disconnect();
                // Clear any pending normalizer timers and run immediately
        ...
        // Start observing each plot for mutations
        const observerConfig = { childList: true, subtree: true };
        plots.forEach(plot => observer.observe(plot, observerConfig));

        // Failsafe: If no rendering detected after 2 seconds, force normalization anyway
        setTimeout(() => {
            observer.disconnect();
            let rows = this.ref.table.getRows();
            for (let i = 0; i < rows.length; i++) {
    ...
    async handleAIIconClick() {
        console.log('AI Icon clicked!');
        const { match } = this.props;
        const datasetName = match.params.dsName;
        
        try {
            // Check if embeddings already exist
            const status = await ragService.checkStatus(datasetName);
            ...
            alert('Failed to check AI status. Please try again.');
        }
    }

    async handleRAGConsent() {
        const { match } = this.props;
        const datasetName = match.params.dsName;
        
        this.setState({ ragInitializing: true });
        
        try {
            // Initialize embeddings
            await ragService.initialize(datasetName);
            ...
            <div>
                    ...
                    <Col md={12} sm={12} xs={12}> 
                        <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                            <Link to={`/ds/${match.params.dsName}/${match.params.dsView}`}>
                                <h3 style={{ margin: 0 }}> {match.params.dsName}</h3>
                            </Link>
                            <span 
                                className="ai-icon-btn" 
                                onClick={this.handleAIIconClick} 
                ...
                */}
                ...
                <RAGConsentModal 
                    ...
                    onConfirm={this.handleRAGConsent}
                    datasetName={match.params.dsName}
                    loading={this.state.ragInitializing}
                />
                <DatasetChat 
                    show={this.state.showChatModal}
                    onHide={() => this.setState({ showChatModal: false })}
                    datasetId={match.params.dsName}
                    datasetName={match.params.dsName}
                />
            </div>
        );
